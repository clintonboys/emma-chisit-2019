...
class Seat(object):
    ...
    def runoff(self, primary_results, pref_flows):
        ...
        remaining_candidates = primary_results.copy()
        round_no = 0
        ...
        while len(remaining_candidates) > 2:
            ...
            for party in remaining_candidates:
                if party in to_dist:
                    fixed_preferences[party] = to_dist[party]
            ...
            for party in remaining_candidates:
                if party in to_dist:
                    remaining_candidates[party] = np.round(remaining_candidates[party] + to_dist[party] * votes, 2)
                else:
