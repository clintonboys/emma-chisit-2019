...
composition_frame = pd.read_csv('/Users/clinton/Documents/dev/emma-chisit-2019/site/current_forecast.csv')
...
for i in range(0, len(composition_frame)):
    try:
        ...
        composition[composition_frame['winner'].iloc[i]] = 1
print composition_frame.groupby('winner').agg('count')
composition['aALP'] = composition.pop('ALP')
composition['bIND'] = composition.pop('IND1')
try:
...
composition['zLIB'] = composition.pop('LIB')
composition['zNAT'] = composition.pop('NAT')

# composition = {'ALP': 55, 'COA': 90, 'IND': 3, 'PUP': 1, 'GRN': 1}
this_time = datetime.datetime.now() - datetime.timedelta(hours=-9)

from_svg = open('/Users/clinton/Documents/dev/emma-chisit-2019/site/emmagraphic5.svg').readlines()
circles = []
...
def GenerateForecastGraphic(composition, this_time, filename):
    colors = {'aALP': color.rgb(1, 10.0 / 255.0, 10.0 / 255.0),
              'bIND': color.rgb(171.0 / 255.0, 171.0 / 255.0, 171.0 / 255.0),
              'cPUP': color.rgb(247.0 / 255.0, 1, 10.0 / 255.0),
              'eGRN': color.rgb(8.0 / 255.0, 242.0 / 255.0, 0),
              'dKAP': color.rgb(1, 166.0 / 255.0, 0),
              'zLIB': color.rgb(59.0 / 255.0, 10.0 / 255.0, 1),
              'zNAT': color.rgb(59.0 / 255.0, 10.0 / 255.0, 1)}
    color_list = []
    ...
    for i in range(0, len(circles)):
        c.fill(path.circle(circles[numbers_map[i]][0] / 150.0, circles[numbers_map[i]][1] / 150.0,
                           circles[numbers_map[i]][2] / 150.0), [color_list[i]])
...
GenerateForecastGraphic(composition, this_time, '/Users/clinton/Documents/dev/emma-chisit-2019/site/imager.pdf')